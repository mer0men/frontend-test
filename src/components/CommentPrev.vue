<template lang="pug">
div.comment-prev
  p.header {{comment.nickname}}
  span.date {{comment.date.getDate()}}.{{comment.date.getMonth() + 1}}.{{comment.date.getFullYear()}}
  div.stars
    svg.star(width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" v-for="n in 5")
      path(:class="{'fill-star': n <= comment.rating}" d="M9.07668 1.21993C9.41827 0.398635 10.5817 0.398636 10.9233 1.21993L12.9395 6.06735C13.0835 6.41358 13.4091 6.65015 13.7829 6.68012L19.0161 7.09966C19.9027 7.17074 20.2623 8.27725 19.5867 8.85592L15.5996 12.2713C15.3148 12.5153 15.1904 12.8981 15.2774 13.2628L16.4956 18.3695C16.702 19.2348 15.7607 19.9186 15.0016 19.455L10.5213 16.7184C10.2012 16.5229 9.79876 16.5229 9.47875 16.7184L4.9984 19.455C4.2393 19.9186 3.29805 19.2348 3.50444 18.3695L4.72257 13.2628C4.80958 12.8981 4.68521 12.5153 4.40042 12.2713L0.413277 8.85592C-0.26226 8.27725 0.0972674 7.17074 0.983922 7.09966L6.21712 6.68012C6.59091 6.65015 6.91652 6.41358 7.06052 6.06735L9.07668 1.21993Z" fill="#C4C4C4")

  p.comment {{comment.comment}}
</template>

<script>
export default {
  props: {
    comment: {
      required: true
    }
  }
}
</script>

<style lang="scss">
.comment-prev {
  position: relative;
  padding: 16px 0;
  border-bottom: 1px solid #D9D9D9;

  &:first-child {
    border-top: 1px solid #D9D9D9;
  }

  .date {
    position: absolute;
    top: 16px;
    right: 16px;
    color: #A79595;
    font-size: 11px;
    line-height: 13px;
  }
}
</style>
